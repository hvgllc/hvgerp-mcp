...
- **The seven viewers now carry Havi Group branding.** The light palette in
  `src/ui/global.css` is no longer invented here: every value is copied from the
  Havi Group Workspace SPA design tokens (accent `#E85D1F`, surface `#F6F7F9`,
  ink `#18202B`, rules `#E6E8EC`, 9px/14px rounding), each annotated with the
  SPA token it came from, so a viewer rendered inside an MCP host reads as part
  of the same product as the workspace itself. Because `src/ui/shared/theme.ts`
  funnels every colour through a CSS custom property, that one file re-skins all
  seven viewers.
